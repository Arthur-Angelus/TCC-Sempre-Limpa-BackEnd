...
// BUSCAR DADOS HOME USUÁRIO
const buscarDadosHome = async function (id) {
    let MESSAGES = JSON.parse(JSON.stringify(DEFAULT_MESSAGES))

    try {
        if (!isNaN(id) && id != '' && id != null && id > 0) {
            let resultUsuarios = await usuarioDAO.selectHomeUsuario(Number(id))
            ...
            if (resultUsuarios) {
                if (resultUsuarios.length > 0) {
                    ...
                    MESSAGES.ERROR_NOT_FOUND.message += "controller buscar dados home usuario id"
                    return MESSAGES.ERROR_NOT_FOUND //404
                }
            } else {
                MESSAGES.ERROR_INTERNAL_SERVER_MODEL.message += "controller buscar dados home usuario id"
                return MESSAGES.ERROR_INTERNAL_SERVER_MODEL //500
            }
        ...
        MESSAGES.ERROR_INTERNAL_SERVER_CONTROLLER.message += "controller buscar dados home usuario id"
        return MESSAGES.ERROR_INTERNAL_SERVER_CONTROLLER //500
    }
}
